...
async function main() {
    ...
    if (!exportedKey) {
        throw new Error('PRIVATE_KEY environment variable is required. Create a .env file with PRIVATE_KEY=suiprivkey1...');
    }
    ...
    const message = {
        from: address,
        custom_message: Array.from(new TextEncoder().encode("I as a server attest this character is in this location")),
        distance: 0n
    };
